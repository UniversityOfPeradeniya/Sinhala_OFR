function[]= segment(inputImage,outputFullPath,PageAndRow)

%image resizing factor
imgHeight = 40;

folderPath = outputFullPath;
...
for i=1:numel(S)
    ...
    %save it
    filename = [folderPath,PageAndRow,int2str(i),'.png'];
    
    imgWidth =  round(size(portion,2)/(size(portion,1)/imgHeight));
    portionResized = imresize(portion,[imgHeight,imgWidth]);
    imwrite(portionResized,filename);
    disp(filename);
